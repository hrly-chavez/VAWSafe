...
const RegisterUser = ({ onClose, defaultRole }) => {
  ...
  useEffect(() => {
    const checkDSWD = async () => {
      try {
        const res = await axios.get("http://10.178.136.238:8000/api/auth/check-dswd/");
  ...
  // Fetch provinces
  useEffect(() => {
    axios.get("http://10.178.136.238:8000/api/desk_officer/provinces/")
  ...
  useEffect(() => {
    if (selectedProvince) {
      axios.get(`http://10.178.136.238:8000/api/desk_officer/provinces/${selectedProvince}/municipalities/`)
  ...
  useEffect(() => {
    if (selectedMunicipality) {
      axios.get(`http://10.178.136.238:8000/api/desk_officer/municipalities/${selectedMunicipality}/barangays/`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // const checkResponse = await fetch("http://10.178.136.238:8000/api/auth/search-official/", {
      //   method: "POST",
      //   body: formData,
      // });
      ...
      const response = await fetch("http://10.178.136.238:8000/api/auth/add-user/", {
        method: "POST",
