...
export default function Sidebar() {
  ...
  const getSidebarItems = () => {
    ...
    if (role === "vawdesk" || role === "desk officer") {
      return [
        ...
        { icon: "/images/account-settings.png", label: "Account Management", path: "/desk_officer/account-management" },
        { icon: "/images/logout.png", label: "Log Out", path: "/login", isLogout: true }
      ];
    }
  ...
  return (
    <aside className="h-screen w-[220px] bg-[#2F2F4F] text-white font-poppins sticky top-0 shadow-lg z-10">
      {/* Profile Section */}
        ...
        {sidebarItems.map((item, idx) => {
          if (item.isLogout) {
            return (
              ...
              >
                <div className="flex items-center gap-3 px-4 py-3 rounded-md transition-all duration-200 ease-in-out hover:bg-[#3F3F64] hover:scale-[1.02] hover:shadow-md">
                  <img
                    src={item.icon}
                    alt={item.label}
