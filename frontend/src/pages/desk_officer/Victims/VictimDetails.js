...
export default function VictimDetails() {
  ...
  // fecth ang full address na working na
  const fullAddress = get(victim, ["full_address"], "—");

  const age = victim?.vic_birth_date
    ? new Date().getFullYear() - new Date(victim.vic_birth_date).getFullYear()
    : null;

  const infoFields = [
    { label: "Email", value: get(victim, ["vic_email"]) },
    { label: "SOGIE", value: get(victim, ["vic_is_SOGIE"]) },
    { label: "Specific SOGIE", value: get(victim, ["vic_specific_sogie"]) },
    { label: "Ethnicity", value: get(victim, ["vic_ethnicity"]) },
      ...
      <div className="relative z-10 pt-[120px] px-6 max-w-screen-lg mx-auto space-y-10">
        ...
        <div className="bg-white border rounded-xl shadow-lg p-6">
          <h3 className="text-lg font-semibold text-[#292D96] mb-4">
            {isMinor ? "Victim & Guardian Information" : "Victim Information"}
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {infoFields
              ...
              .map((item, index) => (
                <div key={index}>
                  ...
                  <div className="bg-gray-50 rounded-md px-4 py-3 border shadow-sm">
                    <p className="text-sm font-medium text-gray-800 uppercase">{item.value}</p>
                  </div>
                </div>
              ))}
          </div>
          ...
          {fullAddress && fullAddress !== "N/A" && (
            <div className="mt-6">
              <p className="text-xs text-gray-500 mb-1">Full Address</p>
              <p className="text-sm font-medium text-gray-800 uppercase bg-gray-50 border rounded-md p-3 whitespace-pre-wrap break-words">
                {fullAddress}
              </p>
            </div>
          )}
        ...
        {incidentList.length > 0 && (
          <div className="bg-white border rounded-xl shadow-lg p-6 mt-10">
            <h3 className="text-lg font-semibold text-[#292D96] mb-4">Case Information</h3>
                        ...
                        {(incident.sessions || []).map((session) => (
                          ...
                          >
                            ...
                            <p className="text-sm">
                              <span className="font-medium">Status:</span>{" "}
                              <span
                                className={`px-2 py-0.5 rounded text-xs ${session.sess_status === "Pending"
                                  ? "bg-yellow-100 text-yellow-700"
                                  : session.sess_status === "Ongoing"
                                    ? "bg-blue-100 text-blue-700"
                                    : session.sess_status === "Done"
