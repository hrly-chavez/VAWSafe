    ...
    >
      <div className="flex items-center justify-center min-h-screen px-10 py-12 text-white">
        <div className="relative w-full max-w-[90rem] h-[550px]  grid grid-cols-1 sm:grid-cols-2 gap-8 items-center bg-white/5 backdrop-blur-lg rounded-xl shadow-2xl p-8">
          {/* Left Description */}
          <div className={`text-white space-y-6 transition-opacity duration-300 ${fade ? "opacity-100" : "opacity-0"}`}>
            <h2 className="text-4xl font-bold">{slides[currentIndex].title}</h2>
            <p className="text-white/90 text-lg">{slides[currentIndex].desc}</p>
            <Link
              to="/login"
              className="inline-block px-6 py-2 bg-gradient-to-r from-orange-500 to-pink-500 text-white rounded-lg shadow-lg font-semibold hover:scale-105 transition-transform"
            >
              Continue
            </Link>
          </div>
