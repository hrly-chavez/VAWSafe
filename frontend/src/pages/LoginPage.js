...
const LoginPage = () => {
  ...
  const handleFaceLogin = async () => {
    ...
    try {
      const blinkRes = await apiFetch(
        "http://10.178.136.238:8000/api/auth/blink-check/",
      ...
      const loginRes = await apiFetch(
        "http://10.178.136.238:8000/api/auth/face-login/",
  ...
  const handleManualLogin = async () => {
    ...
    try {
      const response = await apiFetch("http://10.178.136.238:8000/api/auth/manual-login/", {
  ...
  useEffect(() => {
    const checkDSWD = async () => {
      try {
        const res = await fetch("http://10.178.136.238:8000/api/auth/check-dswd/");
