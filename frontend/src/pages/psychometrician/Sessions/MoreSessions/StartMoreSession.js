    ...
    const fetchData = async () => {
      try {
        const [sessRes, catRes] = await Promise.all([
          api.get(`/api/psychometrician/sessions/${sess_id}/`),
          api.get(`/api/psychometrician/service-categories/`),
        ]);
    ...
    const fetchServices = async () => {
      try {
        const res = await api.get(
          `/api/psychometrician/services/category/${selectedCategory.value}/`
        );
  ...
  const handleFinishSession = async () => {
    ...
    try {
      ...
      if (session?.incident?.vic_id?.vic_id) {
        navigate(`/psychometrician/victims/${session.incident.vic_id.vic_id}`);
      } else {
