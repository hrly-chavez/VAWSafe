...
export default function StartSession() {
  ...
  useEffect(() => {
    api
      .get("/api/psychometrician/service-categories/")
  ...
  useEffect(() => {
    ...
    api
      .get(`/api/psychometrician/services/category/${selectedCategory.value}/`)
  ...
  //     } else {
  //       navigate("/psychometrician/sessions");
  //     }
  //   } catch (err) {
  ...
  try {
    ...
    await api.post(`/api/psychometrician/sessions/${sess_id}/finish/`, payload);
    alert("Your part of the session is now marked as completed.");
    
    // Always redirect, no modal anymore
    navigate("/psychometrician/sessions");
    ...
    <div className="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-md space-y-10">
      ...
      <NextSessionModal
        ...
        onClose={(success) => {
          ...
          if (success) navigate("/psychometrician/sessions");
        }}
        session={session}
      />
      <CaseSessionFollowup
