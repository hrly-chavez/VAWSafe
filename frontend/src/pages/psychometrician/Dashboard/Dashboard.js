...
export default function PsychometricianDashboard() {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                const res = await api.get(`/api/psychometrician/dashboard/summary/?year=${selectedYear}`);
                const data = res.data;
                ...
                setViolenceTypes(data.violence_types || {});
            } catch (err) {
                console.error(err);
    ...
    const lineData = {
        ...
        datasets: [
            {
                label: "Monthly Victims (Psychometrician)",
                data: values,
                borderColor: "#EF4444",
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 font-inter bg-[#f7f9fc]">
            ...
            <header>
                ...
                <p className="text-gray-500 mt-1">Psychological evaluation and progress overview.</p>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Upcoming Sessions */}
                <div className="p-6 bg-red-50 rounded-xl border-4 border-red-400 card-shadow transition hover:shadow-lg">
                    <div className="flex items-center mb-4">
                        <BellIcon className="w-6 h-6 text-red-600 mr-2 animate-pulse" />
                        <h2 className="text-xl font-bold text-red-800">Upcoming Due Sessions</h2>
                    </div>
                    <p className="text-sm text-red-700 mb-4 border-b border-red-200 pb-3">
                        These sessions require immediate psychological evaluation and follow-up.
                    </p>
                        ...
                        <div className="space-y-4">
                            {notifications.length > 0 ? (
                                notifications.map((n) => (
                                    <div
                                        key={n.id}
                                        className="flex justify-between items-start p-3 bg-red-100 rounded-lg hover:bg-red-200 transition"
                                    >
                                        <div className="flex flex-col">
                                            <p className="text-xs font-semibold text-red-900">
                                                Session {n.sess_num} – {n.victim}
                                            </p>
                                            <p className="text-[11px] text-red-700">{n.type}</p>
                                        </div>
                                ...
                                <p className="text-sm text-red-600">No upcoming sessions.</p>
                            )}
                        </div>
                    </div>

                    <Link
                        to="/psychometrician/sessions"
                        className="block mt-6 text-center text-sm font-semibold text-red-600 hover:text-red-900 transition"
                    >
                ...
                <div className="p-6 bg-red-50 rounded-xl border-4 border-red-400 card-shadow transition hover:shadow-lg">
                    <div className="flex items-center mb-4">
                        <BellIcon className="w-6 h-6 text-red-600 mr-2 animate-pulse" />
                        <h2 className="text-xl font-bold text-red-800">Overdue Sessions</h2>
                    </div>
                    <p className="text-sm text-red-700 mb-4 border-b border-red-200 pb-3">
                    ...
                    <div className="max-h-80 overflow-y-auto pr-2 scrollbar-hide">
                        <div className="space-y-4">
                            {overdueSessions.length > 0 ? (
                                overdueSessions.map((s) => (
                                    ...
                                    >
                                        <div className="flex flex-col">
                                            ...
                                            <p className="text-[11px] text-red-700">{s.type}</p>
                                        </div>
                                        <div className="flex flex-col items-end gap-1">
                                            <p className="text-[11px] font-medium text-red-800">
                                            ...
                                            <Link
                                                to={`/psychometrician/sessions/${s.id}`}
                                                className="text-[11px] text-red-600 hover:text-red-800 underline"
                                            >
                                                View
                                            </Link>
                                ...
                                <p className="text-sm text-red-600">No overdue sessions.</p>
                            )}
                        </div>
                    </div>
