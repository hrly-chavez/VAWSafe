// src/pages/nurse/Sessions/MoreSessions/CreateSession.js
// Creating Session 2+
    ...
    const fetchData = async () => {
      try {
        const [summaryRes, typeRes] = await Promise.all([
          api.get(`/api/nurse/incident/${incident_id}/summary/`),
          api.get(`/api/nurse/session-types/`),
        ]);
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      try {
        ...
        const url = `/api/nurse/mapped-questions/?session_num=${summary.next_session_number}&session_types=${selectedType.value}&role_filter=1`;
