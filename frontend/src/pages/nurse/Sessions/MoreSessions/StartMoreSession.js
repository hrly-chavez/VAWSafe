// src/pages/nurse/Sessions/MoreSessions/StartMoreSession.js
    ...
    const fetchData = async () => {
      try {
        const [sessRes, catRes] = await Promise.all([
          api.get(`/api/nurse/sessions/${sess_id}/`),
          api.get(`/api/nurse/service-categories/`),
        ]);
    ...
    const fetchServices = async () => {
      try {
        const res = await api.get(
          `/api/nurse/services/category/${selectedCategory.value}/`
        );
