// src/pages/nurse/Sessions/ViewSessions.js
...
export default function ViewSessions() {
  ...
  useEffect(() => {
  api
    .get(`/api/nurse/sessions/${sess_id}/`)
    .then((res) => {
      const data = res.data;
      setSession(data);
  ...
  const handleSaveType = async () => {
    try {
      ...
      const res = await api.patch(`/api/nurse/sessions/${sess_id}/`, {
        sess_type: ids,
      });
      setSession(res.data);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-md space-y-8">
      ...
      {victim && (
        <div>
          <Link
            to={`/nurse/victims/${victim.vic_id}`}
