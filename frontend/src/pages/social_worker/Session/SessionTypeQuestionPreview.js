...
export default function SessionTypeQuestionPreview({ sessionNum, selectedTypes }) {
  ...
  useEffect(() => {
    if (!sessionNum || !selectedTypes.length) {
      setQuestions([]);
      return;
    }

    const typeIds = selectedTypes.map((t) => t.value).join(",");
    api
      .get(`/api/desk_officer/mapped-questions/?session_num=${sessionNum}&session_types=${typeIds}`)
