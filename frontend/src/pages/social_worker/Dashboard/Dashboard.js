...
export default function SocialWorkerDashboard() {
  ...
  const lineData = {
    labels: monthlyReports.map(r => r.month),
    datasets: [
      {
        label: "Victims Registered",
        data: monthlyReports.map(r => r.totalVictims),
        borderColor: "#F59E0B",
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 font-inter bg-[#f7f9fc]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Upcoming Sessions */}
        <div className="p-6 bg-amber-50 rounded-xl border-4 border-amber-400 card-shadow transition hover:shadow-lg">
          ...
          <div className="max-h-80 overflow-y-auto pr-2 scrollbar-hide">
            <div className="space-y-4">
              {upcomingSessions.length > 0 ? (
                upcomingSessions.map((n) => (
                  <div
                    key={n.id}
                    className="flex justify-between items-start p-3 bg-amber-100 rounded-lg hover:bg-amber-200 transition"
                  >
                    <div className="flex flex-col">
                      <p className="text-xs font-semibold text-amber-900">
                        Session {n.sess_num} – {n.victim}
                      </p>
                      <p className="text-[11px] text-amber-700">{n.type}</p>
                    </div>
                ...
                <p className="text-sm text-amber-600">No upcoming sessions.</p>
              )}
            </div>
          </div>
        ...
        <div className="p-6 bg-amber-50 rounded-xl border-4 border-amber-400 card-shadow transition hover:shadow-lg">
          <div className="flex items-center mb-4">
            <BellIcon className="w-6 h-6 text-amber-600 mr-2 animate-pulse" />
            <h2 className="text-xl font-bold text-amber-800">Overdue Sessions</h2>
          </div>
          <p className="text-sm text-amber-700 mb-4 border-b border-amber-200 pb-3">
          ...
          <div className="max-h-80 overflow-y-auto pr-2 scrollbar-hide">
            <div className="space-y-4">
              {overdueSessions.length > 0 ? (
                overdueSessions.map((s) => (
                  ...
                  >
                    <div className="flex flex-col">
                      ...
                      <p className="text-[11px] text-amber-700">{s.type}</p>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      <p className="text-[11px] font-medium text-amber-800">
                      ...
                      <Link
                        to={`/social_worker/sessions/${s.id}`}
                        className="text-[11px] text-amber-600 hover:text-amber-800 underline"
                      >
                        View
                      </Link>
                ...
                <p className="text-sm text-amber-600">No overdue sessions.</p>
              )}
            </div>
          </div>
