...
export default function SocialWorkerDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const res = await api.get(`/api/social_worker/dashboard/summary/`);
        const data = res.data;
        ...
        setCount(
          (data.upcoming_sessions?.length || 0) +
            (data.overdue_sessions?.length || 0)
        );
      } catch (err) {
        console.error(err);
  ...
  const kpiCards = [
    {
      ...
      value: victimSummary.total_victims ?? 0,
      change: victimSummary.total_victims_percent
        ? `${victimSummary.total_victims_percent}%`
        : null,
      changeDirection:
        victimSummary.total_victims_percent > 0 ? "up" : "neutral",
    ...
    },
    {
      ...
      change:
        sessionSummary.sessions_week_percent !== undefined
    ...
    },
    {
      ...
      change:
        sessionSummary.pending_percent !== undefined
          ? `${sessionSummary.pending_percent}%`
          : null,
      changeDirection: sessionSummary.pending_percent > 0 ? "up" : "neutral",
    ...
    },
    {
      ...
      change:
        sessionSummary.ongoing_percent !== undefined
          ? `${sessionSummary.ongoing_percent}%`
          : null,
      changeDirection: sessionSummary.ongoing_percent > 0 ? "up" : "neutral",
    ...
    <div className="p-4 md:p-8 space-y-8 font-inter bg-[#f7f9fc]">
      ...
      <header>
        ...
        <p className="text-gray-500 mt-1">
          Track consultations, session progress, and key notifications at a
          glance.
        </p>
      </header>
              ...
              >
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    {kpi.icon}
                    <h3 className="text-gray-600 text-sm font-semibold tracking-wide">
                      {kpi.label}
                    </h3>
                  </div>
                  {kpi.change && (
              ...
              >
                <div className="flex justify-between items-start bg-white border border-gray-300 p-4 rounded-lg shadow-sm hover:shadow-md transition cursor-pointer">
                  ...
                  <div className="flex flex-col gap-2">
                    {/* Type Label */}
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-semibold text-white bg-${n.color}-500 w-fit`}
                    >
                      {n.typeLabel}
                    </span>
