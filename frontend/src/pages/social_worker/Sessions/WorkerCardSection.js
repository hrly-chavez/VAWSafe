...
export default function WorkerCardSection({
  ...
  // --- Helper: Get roles of currently selected officials ---
  const selectedRoles = officials
    .filter((o) => selectedOfficials.includes(o.of_id))
    .map((o) => o.role);
      ...
      {loadingSW ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
          {officials.map((worker) => {
            ...
            // Determine if worker role is already taken
            const isRoleTaken =
              selectedRoles.includes(worker.role) &&
              ...
              <div
                ...
                className={`border rounded-lg p-4 bg-white shadow-sm transition ${
                  selectedOfficials.includes(worker.of_id)
                    ...
                    : "hover:shadow-md"
                } ${isRoleTaken ? "opacity-50" : ""}`} // Optional gray-out style
              >
                {/* Header Info */}
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-sm font-semibold transition ${
                      selectedOfficials.includes(worker.of_id)
                        ...
                        : "bg-blue-100 text-blue-700 hover:bg-blue-200 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed"
                    }`}
                  >
                    {selectedOfficials.includes(worker.of_id)
                      ? "Selected"
