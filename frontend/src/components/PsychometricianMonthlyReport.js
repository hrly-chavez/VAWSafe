...
const checkboxGroups = {
  ...
  psychological_testing: ["TONI 4", "NEO PI-R", "BPI"],
};

export default function PsychometricianMonthlyReport({
  victim,
  incident,
  onSubmit,
}) {
  const [formData, setFormData] = useState({
    ...Object.keys(checkboxGroups).reduce(
      (acc, key) => ({ ...acc, [key]: [], [`${key}_other`]: "" }),
      {}
    ),
    safety_issues: [],
    safety_issues_other: "",
  ...
  return (
    <form onSubmit={handleSubmit} className="text-sm text-gray-700 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-2 mt-2">
            {checkboxGroups[group].map((opt) => (
              <label key={opt} className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={formData[group].includes(opt)}
                  onChange={() => toggleCheckbox(group, opt)}
                />
                {opt}
              </label>
            ))}
      ...
      <div className="bg-white border rounded-lg shadow-sm p-4">
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          {["None", "Suicidal Ideation", "Homicidal Ideation", "Other"].map(
            (opt) => (
              <label key={opt} className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={formData.safety_issues.includes(opt)}
                  onChange={() => toggleCheckbox("safety_issues", opt)}
                />
                {opt}
              </label>
            )
        ...
        <input
          type="text"
          name="safety_issues_other"
          placeholder="Other..."
          value={formData.safety_issues_other}
          onChange={handleChange}
          className="mt-2 w-full border rounded-md p-2 text-sm"
        />
      </div>
      ...
      {formData.safety_issues.some((issue) =>
        ["Suicidal Ideation", "Homicidal Ideation", "Other"].includes(issue)
      ) && (
        <div className="bg-white border rounded-lg shadow-sm p-4">
          ...
          <div className="grid grid-cols-2 gap-2 mt-2">
            {["Intention to act", "Plan to act", "Means to act"].map((opt) => (
              <label key={opt} className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={formData.client_has.includes(opt)}
                  onChange={() => toggleCheckbox("client_has", opt)}
                />
                {opt}
              </label>
            ))}
      ...
      {[
        ...
        {
          name: "latest_checkup_psychologist",
          label: "Latest Check-up with Psychologist",
        },
        {
          name: "latest_checkup_psychiatrist",
          label: "Latest Check-up with Psychiatrist",
        },
      ].map(({ name, label }) => (
        <div key={name}>
          ...
          <textarea
            ...
            value={formData[name]}
            onChange={handleChange}
            className="w-full border rounded-md p-3 text-sm text-gray-800 resize-none h-24"
          />
        </div>
      ))}
      ...
      <div className="bg-white border rounded-lg shadow-sm p-4">
        <SectionHeader
          icon="/images/clipboard.png"
          title="Psychological Testing (if conducted)"
        />
        <div className="grid grid-cols-2 gap-2 mt-2">
