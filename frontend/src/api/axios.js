...
// Helper: read a cookie value (for CSRF)
...
// Attach CSRF token for unsafe methods (Django expects X-CSRFToken)
api.interceptors.request.use((config) => {
...
// Optional: on 401, you could try silent refresh then retry once
...
api.interceptors.response.use(
  ...
  async (err) => {
    ...
    if (err.response?.status === 401 && !original._retry) {
      if (isRefreshing) {
        // queue while a refresh call is in-flight
      ...
      try {
        isRefreshing = true;
        original._retry = true;
        await api.post("/api/auth/refresh/"); // cookies in, cookies out
        pending.forEach((r) => r());
        pending = [];
        return api(original);
      } catch (e) {
