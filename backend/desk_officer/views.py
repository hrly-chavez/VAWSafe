...
#         if not photo_files:
#             print("[WARN] No photo(s) provided for victim")
#             return Response({"error": "At least one face photo is required."},
#                             status=status.HTTP_400_BAD_REQUEST)
...
#                     enforce_detection=True  # strict like Official
#                 )

#                 # Normalize DeepFace output
#                 if isinstance(reps, list):
#                     if isinstance(reps[0], dict) and "embedding" in reps[0]:
#                         embedding_vector = reps[0]["embedding"]
#                     elif all(isinstance(x, float) for x in reps):
#                         embedding_vector = reps
#                     else:
#                         raise ValueError("Unexpected embedding format")
#                 elif isinstance(reps, dict) and "embedding" in reps:
#                     embedding_vector = reps["embedding"]
#                 else:
#                     raise ValueError("Unexpected DeepFace response")
...
#                 )
#                 created_count += 1
#                 print(f"[INFO] Face sample #{idx} saved for victim {victim.vic_id}")

#             except Exception as e:
#                 print(f"[ERROR] Failed to process face sample #{idx}: {e}")
#                 traceback.print_exc()
#             finally:
